...
tags: [python, kiwoom, api, stock, 개발기]
comments: true
excerpt_separator: <!--more-->
---
안녕하세요! 이번 포스팅부터는 파이썬(Python)과 **키움증권 REST API**를 활용하여 저만의 맞춤형 주식 분석 툴을 만들어가는 과정을 시리즈로 연재해보려고 합니다. 

우리가 궁극적으로 만들게 될 프로그램은 주식 차트, 보조지표, 외국인/기관 수급, 그리고 실시간 웹 검색을 결합한 AI 챗봇까지 포함된 **통합 주식 대시보드**입니다. 

하지만 천 리 길도 한 걸음부터! 가장 먼저 해야 할 일은 API 서버와 통신하기 위한 **접근 토큰(Access Token)**을 발급받는 것입니다. 매번 수동으로 토큰을 발급받는 것은 번거로우므로, 파이썬 스크립트를 통해 자동으로 토큰을 발급받고 관리하는 `get_token.py` 모듈을 만들어 보겠습니다.
