...
차트 엔진을 가속하기 위해선 `C++`를 기반으로 하는 `Qt`같은 툴을 이용하여 설계하는 것이 더 바람직하지만 저는 **스윙 매매**를 위해 이 툴을 제작하고 있기 때문에 `Python`을 활용하는 `Pyside6`를 통해 UI를 구현하였습니다. 이 시리즈를 통해 기본적인 차트툴의 뼈대를 완성하고, 각자 원하는 매매전략을 더함으로써 **나만의 차트툴**을 만들어 보도록 합시다 🔥🔥
...
프로젝트 폴더 최상단에 `.env` 파일을 만들고 아래와 같이 입력해 줍니다.
